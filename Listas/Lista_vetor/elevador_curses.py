...
def parar_nos_andares():
    ...
    if direcao_elevador == 'subindo':
        if int(andar_atual) in fila_subir:
            ...
            if (fila_subir) and (fila_descer[0] > fila_subir[-1]): #
        ...
        if int(andar_atual) in fila_descer:
            ...
            if (fila_descer) and (fila_descer[0] > fila_subir[-1]): #
                fila_subir.remove(int(andar_atual))
...
def movimentar_elevador():
    ...
    min_subindo = min(fila_subir) if fila_subir else float('+inf') #
    min_descendo = min(fila_descer) if fila_descer else float('+inf') #
    valor_minimo = min(min_subindo, min_descendo) #
